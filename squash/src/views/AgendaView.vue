  ...
  zones.forEach((zone) => {
    ...
    if (
      ...
      for (const task of tasks) {
        ...
        if (
          !(e.clientY >= task.getBoundingClientRect().top + task.getBoundingClientRect().height / 2)
        ) {
          if (!aboveTask.value) {
...
const dragEnd = async () => {
  ...
  const currentTaskIndex = taskRanksDeepCopy[getDateByZone(hoveredZone.value)].findIndex(
    (taskId) => taskId === store.currentDraggedTask.id,
  );
  ...
  if (currentTaskIndex !== -1) {
    ...
    const currentTaskRanks =
      taskRanksDeepCopy[dayjs(store.currentDraggedTask.date).format('YYYY-MM-DD')];
    currentTaskRanks.splice(
      currentTaskRanks.findIndex((taskId) => taskId === store.currentDraggedTask.id),
      1,
    );
  }

  const aboveTaskIndex = taskRanksDeepCopy[getDateByZone(hoveredZone.value)].findIndex(
    (taskId) => taskId === Number(aboveTask.value?.getAttribute('data-id')),
  );

  if (aboveTaskIndex === -1) {
    taskRanksDeepCopy[getDateByZone(hoveredZone.value)].push(store.currentDraggedTask.id);
  } else {
    taskRanksDeepCopy[getDateByZone(hoveredZone.value)].splice(
      aboveTaskIndex,
      0,
      store.currentDraggedTask.id,
    );
  }

  store.updateTaskRanks(
    taskRanksDeepCopy[getDateByZone(hoveredZone.value)],
    getDateByZone(hoveredZone.value),
  );
  if (store.currentDraggedTask.date !== getDateByZone(hoveredZone.value)) {
    store.updateTaskRanks(
      taskRanksDeepCopy[dayjs(store.currentDraggedTask.date).format('YYYY-MM-DD')],
      dayjs(store.currentDraggedTask.date).format('YYYY-MM-DD'),
    );
  }
...
const tomorrow = computed(() => {
  ...
  return (store.taskRanks[dayjs().add(1, 'day').format('YYYY-MM-DD')] || []).map(
    (taskId) => tasks.find((task) => task.id === taskId) || {},
  );
});
...
const week = computed(() => {
  ...
  return (store.taskRanks[dayjs().add(7, 'day').format('YYYY-MM-DD')] || []).map(
    (taskId) => tasks.find((task) => task.id === taskId) || {},
  );
});
</script>
  ...
  <main>
    <div class="agenda">
      <div class="current-tasks">
        <AgendaContainer
          :tasks="today"
          :zone="AgendaZones.TODAY"
          :hovered="hoveredZone === AgendaZones.TODAY"
          @dragEnd="dragEnd"
          @drag="drag" />
        <AgendaContainer
          :tasks="tomorrow"
          :zone="AgendaZones.TOMORROW"
          :hovered="hoveredZone === AgendaZones.TOMORROW"
          @dragEnd="dragEnd"
          @drag="drag" />
        <AgendaContainer
          :tasks="week"
          :zone="AgendaZones.THIS_WEEK"
          :hovered="hoveredZone === AgendaZones.THIS_WEEK"
          @dragEnd="dragEnd"
          @drag="drag" />
      </div>
    </div>
